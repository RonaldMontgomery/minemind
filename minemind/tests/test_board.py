...
# ---------------------------------------------------------------------------
# Cell and Board configuration tests
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Neighbor and mine placement tests
# ---------------------------------------------------------------------------
...
class TestNeighborCounts(unittest.TestCase):
    ...
    def test_calculate_neighbor_counts_simple_pattern(self):
        ...
        self.assertEqual(b.cells[1][0].neighbor_mines, 2)  # near (0,0) and (2,1)
...
# ---------------------------------------------------------------------------
# Solver: trivial rule tests (T1–T4)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Solver: trivial + subset rule tests (T1–T6)
# ---------------------------------------------------------------------------
...
class TestSolverTrivialAndSubsetRules(unittest.TestCase):
    ...
    # ----- Trivial rules again (T1–T4) for slightly different patterns -----
    ...
    # ----- Subset logic (T5–T6 + your Snippet 15) -----
    ...
    def test_subset_basic_reveal(self):
        ...
        Since Hidden_A ⊂ Hidden_B and remaining_mines are equal,
        the difference D = Hidden_B - Hidden_A = { (2,1) } must be all safe.
        ...
        # Frontier cells A, B must be revealed numbered cells
    ...
    def test_subset_basic_flag(self):
        ...
        remaining_mines_A = 1, remaining_mines_B = 2
        extra_mines = remaining_mines_B - remaining_mines_A = 1
        D = Hidden_B - Hidden_A = { (2,1) }, |D| = 1
        ...
        # No flags; just encode counts directly
        b.cells[A[0]][A[1]].neighbor_mines = 1
    ...
    def test_subset_reveal_rule(self):
        ...
        # Key assertion from your snippet:
        self.assertIn((2, 1, "REVEAL"), moves)
        # And make sure we don't flag it
        self.assertNotIn((2, 1, "FLAG"), moves)
